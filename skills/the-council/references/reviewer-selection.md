...
|---|---|---|
| **Routine** | ordinary diffs, bug fixes, refactors, single-file changes | **Fable (low)** + **Codex** + **Gemini** |
...
Codex + Gemini on every round, mandatory unless Tom says otherwise; **Qwen / GLM only when Tom asks** (Tom 2026-09-19). No tier adds them automatically.
...
```
Routine change     → Fable(low) + Codex + Gemini
Hard-to-reverse    → Fable(low) + Codex + Gemini
Milestone / plan   → Fable(low) + Codex + Gemini; Fable(high) at merge
Tom asks for them  → + Qwen / GLM (--openrouter qwen,glm: newest on listing, :floor host, 32k tokens)
